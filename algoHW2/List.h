...
public:
    class Node {
    public:
        Node(T x = 0, Node* ptr = NULL) { data = x; next = ptr; freqVal = 1; }
        T getData() { return data; }
        int getFreq() { return freqVal; }
        Node* getNext() { return next; }
        void setData(T x) { data = x; freqVal++; }
        void setNext(Node* ptr) { next = ptr; }
        void setFreq(int freq) { freqVal = freq; }
        void incNode() { freqVal++; }
    ...
    Node* searchMinFreq() {
        ...
        Node* minFreq = currentPtr;
        
        while (currentPtr != tail_ptr) {
            if (minFreq->getData()->getRoot()->getData() > \
                currentPtr->getData()->getRoot()->getData() )
                minFreq = currentPtr;
            currentPtr = currentPtr->getNext();
        }
        return minFreq;
